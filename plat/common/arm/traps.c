...
#endif
#ifdef CONFIG_FORWARD_INSTRUCTION_ABORT
	asm volatile("mrs %0, esr_el1" : "=r"(esr_el1));
	// Check for instruction abort
	if ((esr_el1 & 0xFC000000) == 0x80000000) {
		// Check if it was a translation fault
		if ((esr_el1 & 0x4) == 0x4) {
			extern void uk_upper_level_instruction_abort_handler(
			    unsigned long *register_stack);
			uk_upper_level_instruction_abort_handler(
			    (unsigned long *)regs);
			return;
		}
	}
